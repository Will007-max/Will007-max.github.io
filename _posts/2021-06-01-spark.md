...
Comparing with what  Hadoop MapReduce is known for, 

considering the fact that the data is stored in a cache, we can ask ourselves the question of whether something is lost in terms of guarantee in case a node crashes or a job does not run.

There are two mechanisms implemented in Spark to deal with failure: fault tolerance
and high availability.

**Fault tolerance** is the ability for Spark to **restart a job when the machine fails**, it keeps track of the work and is able
to go back at a certain point. Fault tolerance: If a machine crashes for some
reason, Spark will keep track of the job that was running, even restarting it
from a certain point, and not starting from scratch. It is able to restart the
job and guarantee the result behind it.
