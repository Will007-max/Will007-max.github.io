...
Talking about Spark cluster, a very important element is the cluster manager.
A **cluster manager** deals with the resource allocation to get the tasks done.
It will choose which computations to allocate to which machines and at which
quantities.


- Spark is **faster** than Hadoop since Spark does the processing in the
**main memory** of the worker nodes and prevents unnecessary operations with the disks.
